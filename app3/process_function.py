...
def process_data(file_name,sheet_name):
    ...
    'OPER_NAME': 'IND_STEP', 'FACTORY_PRODUCT_NAME': 'DPN', 'CAPACITY': 'CAP','HEADS_NUM':'HEADS','DISCS_NUM':'DISCS'})
        result = result[result['IND_STEP'].isin(['INSERTION_TP'])]
    ...
    result = result[result['MEAS'].isin(['THP_YIELD','THP YIELD'])]
    ...
    result['DISCS'].fillna(0)
    result['DISCS'] = result['DISCS'].astype(float).astype(int,errors='ignore')
